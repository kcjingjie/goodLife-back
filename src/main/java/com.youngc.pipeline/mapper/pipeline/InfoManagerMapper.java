...
public interface InfoManagerMapper {
     ...
     * @return
     */
    @Insert(" INSERT INTO dev_info (unit_id,image_id,device_type,device_equip, device_alias,device_name,device_desc,status,add_person, add_time, last_person, last_time)" +
            " VALUES(#{unitId}, #{imageId},#{deviceType},#{deviceEquip}, #{deviceAlias},#{deviceName}, #{deviceDesc},#{status},#{addPerson}, now(), #{lastPerson}, now())")
     ...
     * @return
     */
    @Select(" SELECT device_id,unit_id,image_id,device_type,device_equip,device_alias,device_name,device_desc,status " +
     ...
     * 修改设备信息
     */
    @Update(" UPDATE dev_info SET image_id=#{imageId},device_type=#{deviceType},device_equip=#{deviceEquip}, device_alias = #{deviceAlias},device_name=#{deviceName},device_desc=#{deviceDesc}," +
